...
v3.12（專屬 Logo）：App 圖示升級為朱印（印章）風格——朱紅圓角方章＋和紙刻印框＋粗體「道」，
與 App 內蓋章 motif 一致。`public/favicon.svg` 為設計源，`scripts/gen-icons.mjs` 重生成 PWA／
Android 圖示；Android adaptive 背景色改朱紅（光柵 mipmap 前景需本機跑 `@capacitor/assets`）。
