...
```
src/
  data/       內容＝唯一事實來源：kana(142)・vocab(~300 N5)・sentences・pairs・pitch・passages・kaiwa(即時応答/発話表現)・dialogues(情境會話)・kanjiWrite(漢字書寫字集，取自 vocab)
...
測試：`npm test` 157/157（對真原始碼）、`npm run test:e2e` 44/44（Playwright 瀏覽器點按，
...
`npm run build`（strict 綠燈）＋ `npm test`（157/157）＋ `npm run test:e2e`（46/46）
