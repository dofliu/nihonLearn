...
v3.23（加練輪替曝光＋金印獎勵）：使用回饋兩項，皆純獎勵/UI，不動蓋章門檻。①**今日頁「今日の加練」
每日主推一項**——原本一排 +α 鈕（書寫/測驗/重音/文型）越列越長，改為每天輪替主推一項
（`lib/activity.ts` `dailyExtraFeature(dayIndex)` 純函式在 `EXTRA_FEATURES` 內循環，每項都輪得到曝光），
想看全部再點「展開全部 ▾」。畫面清爽、冷門功能也有出場機會。②**金印**——核心五修行全達標＝一般
朱印「済」；當天再做**任一加練**（write/quiz/pitch/pattern）＝金色「済」印（`.hanko.gold`／大印
`.inner.gold`）。判定＝已蓋章 ∩ 當天有加練活動（`goldStampDays`/`hasExtraOnDay` 純函式，讀
`activityLog`，**不新增 Dexie 版本、不改蓋章邏輯**）；蓋章後才做加練也會即時升金印。願意多練給成就感，
**不逼、不扣分、不卡蓋章**（呼應低門檻優先原則）。純函式進 `tests/integration.ts`，金印渲染與加練
展開的 e2e 進 `stamp.spec`/`pattern.spec`/`activity.spec`（`expandExtras` 共用步驟）。核心內容輪替
（第三案，動 TASKS 與各 view 計數判定）刻意留給後續單獨 PR。

測試：`npm test` 181/181（對真原始碼）、`npm run test:e2e` 48/48（Playwright 瀏覽器點按，
...
`npm run build`（strict 綠燈）＋ `npm test`（181/181）＋ `npm run test:e2e`（48/48）
