...
v3（Android 上架）Phase 0 已完成：sidecar base URL 可設定（`lib/sidecar.ts`＋設定頁）、
TTS 音檔快取改 Dexie v3 `ttsCache`（取代 SW CacheFirst）。見 `docs/ANDROID_RELEASE_PLAN.md`。

測試：`npm test` 40/40（對真原始碼）、`npm run test:e2e` 24/24（Playwright 瀏覽器點按）、
`sidecar/test_score.py` 4/4（假 whisper 注入）、`npm run build` strict 綠燈。
...
- **Dexie 版本**：schema 已到 `version(3)`（`ttsCache`）。再改 schema 要 `version(4)` 並處理升級，勿改動舊版定義；`e2e/db.spec.ts` 斷言的 IDB 版本（version×10）要同步改。
...
`npm run build`（strict 綠燈）＋ `npm test`（40/40）＋ `npm run test:e2e`（24/24）
＋（動到 sidecar 時）`python sidecar/test_score.py`。
