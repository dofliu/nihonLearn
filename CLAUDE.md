...
```
src/
  data/       內容＝唯一事實來源：kana(142)・vocab(~190 N5)・sentences・pairs・pitch・passages・kaiwa(即時応答/発話表現)
...
（規劃中 v3.10：LLM 只寫「中文理解題/選項」疊在已驗證短文上，經審核採用後入段落池——日文題材不讓 LLM 生。）

測試：`npm test` 103/103（對真原始碼）、`npm run test:e2e` 37/37（Playwright 瀏覽器點按，
...
`npm run build`（strict 綠燈）＋ `npm test`（103/103）＋ `npm run test:e2e`（37/37）
