...
測試：`npm test` 131/131（對真原始碼）、`npm run test:e2e` 41/41（Playwright 瀏覽器點按，
...
- **Dexie 版本**：schema 已到 `version(6)`（`userListenQ`）。再改 schema 要 `version(7)` 並處理升級，勿改動舊版定義；`e2e/db.spec.ts` 斷言的 IDB 版本（version×10，現為 60）與新表清單要同步改。
...
`npm run build`（strict 綠燈）＋ `npm test`（131/131）＋ `npm run test:e2e`（41/41）
