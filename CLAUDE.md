...
src/
  ...
  lib/        date・importV1（v1→v2 遷移 + 備份匯出）・content（生成 client + 持久化審核佇列 + 採用）・articles（NHK Easy 導入 client + 採用）・coverage（覆蓋率檢核，無依賴）・pitch（mora 切分 + 東京式 pattern）・sidecar（base URL 抽象 + probeHealth）
...
測試：`npm test` 40/40（對真原始碼）、`npm run test:e2e` 26/26（Playwright 瀏覽器點按，
...
`npm run build`（strict 綠燈）＋ `npm test`（40/40）＋ `npm run test:e2e`（26/26）
