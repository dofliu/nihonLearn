...
最後更新：v3.29（自由対話：AI 角色扮演文字輸入版——「🔴 互動深化」第 1 步。場景／開場白沿用
已驗證的 `data/dialogues.ts`，後續對方台詞由 Gemini 依你打的日文即時生成＋每回合一行中文小提示；
`lib/roleplay.ts`＋`lib/llmParse.ts parseRoleplayTurn` 為純函式，AI 產物僅供參考、不入庫、不計蓋章，
無金鑰時固定腳本会話照常可用）。
...
> 生成式的來回互動目前有兩處——AI 助教（`TutorView`，自由聊天）與自由対話（`RoleplayView`，
> v3.29 情境角色扮演）。這個方向要把「來回互動」的模式擴散到更多練習場景。
...
1. ~~**情境角色扮演對話（文字輸入版）**~~（v3.29 完成：`views/RoleplayView.tsx`＋`lib/roleplay.ts`，
   ...
   **可續做的小增量**：①目前不記入学習記録（沒加進 `activity.ts` EXTRA_FEATURES，故也不影響金印）——
   ...
   AI 生成日文的免責文案要一樣顯眼。
2. **AI 助教「主動出題／考我」模式**：`TutorView` 現在是你問它答（被動）。加一個「考我」按鈕：
   助教依你已學詞彙隨機給一個中文情境提示，你打字（或口說）回答，助教即時中文講評＋建議更道地的
   說法，可「換一題」。
3. **跟讀＋即時追問**（`SpeakView` 延伸）：跟讀完一句範例句後，若已設 Gemini 金鑰，AI 順勢問一個
   跟這句情境相關的簡單延伸問題，逼你臨場組句而非只跟讀既有稿子。建議做成「+α 選配」，不動既有
   「口」任務蓋章判定。
...
- **測試**：`npm test` 230/230、`npm run test:e2e` 54/54、`sidecar/test_score.py` 4/4、`test_article.py` 13/13、`npm run build` strict 綠燈。
