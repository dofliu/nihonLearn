...
最後更新：v3.30（AI 助教「考我」模式——「🔴 互動深化」第 2 步。助教出**中文情境題**、你自己用日文
作答，題目與參考答案全部來自已驗證資料（`data/sentences` 壱／弐級例句＋`data/patterns`×已學詞），
LLM 只用**中文**講評你寫的句子；無金鑰時退回「看參考答案自評」照樣能練）。
v3.29（自由対話：AI 角色扮演，PR #37）與 v3.30 為同期兩支獨立分支、各自延伸自 v3.28，互不依賴。
...
> 生成式的來回互動目前集中在 AI 助教（`TutorView`：自由聊天＋v3.30 的「考我」主動造句）。
> 這個方向要把「來回互動」的模式擴散到更多練習場景，而非侷限在助教一處。
...
1. **情境角色扮演對話（文字輸入版）**：在既有固定腳本会話（`data/dialogues.ts` 7 場景／
   `DialogueView`）之外，新增一個「自由對話」模式——同樣的情境設定，但對方句改由 Gemini
   即時生成、依你打字輸入的回應調整下一句（聊天氣泡 UI），每回合附一行中文小提示（用詞恰不恰當、
   有沒有更道地的說法）。先做文字輸入版，語音版留到後面步驟。
...
- **測試**：`npm test` 234/234、`npm run test:e2e` 53/53、`sidecar/test_score.py` 4/4、`test_article.py` 13/13、`npm run build` strict 綠燈。
