...
最後更新：v3.31（跟讀＋即時追問——「🔴 互動深化」第 3 步。跟読分頁例句卡下方新增「追問 ─ AI に聞かれる」：
AI 針對當下那句**已驗證教材例句**的情境追問一句日文，你臨場自己組句打字回答，再拿到**中文**講評；
同一句最多 3 次、換句自動重置；選配加練不計蓋章，無金鑰時整塊只顯示說明、跟讀評分照常）。
前一版 v3.30（AI 助教「考我」模式——「🔴 互動深化」第 2 步。助教出**中文情境題**、你自己用日文
...
>
> 現況：聞き取り／N5 測驗／文型ドリル本質上都是「選擇題」或「跟讀固定稿」，屬被動接收；
> 生成式的來回互動目前有四處——AI 助教的自由聊天與「考我」主動造句（`TutorView`，v3.30）、
> 自由対話（`RoleplayView`，v3.29 情境角色扮演），以及跟讀後的即時追問（`components/FollowUp.tsx`，
> v3.31）。這個方向要把「來回互動」的模式擴散到更多練習場景。
...
1. ~~**情境角色扮演對話（文字輸入版）**~~（v3.29 完成：`views/RoleplayView.tsx`＋`lib/roleplay.ts`，
   ...
   **可續做的小增量**：①目前不記入学習記録（沒加進 `activity.ts` EXTRA_FEATURES，故也不影響金印）——
   若要記，加一個 `roleplay` feature key、同步 `FEATURE_LABEL` 與 `TodayView` 的 `EXTRAS` 輪替，
   並補 activity 測試；②今日頁目前沒有自由対話入口（只能從話す▸会話進），可考慮納入「+α 選配」輪替；
...
2. ~~**AI 助教「主動出題／考我」模式**~~（v3.30 完成：`TutorView` 加 `.lvTabs` 兩分頁「💬 問問題／
   ...
   **可續做的小增量**：①口說作答（目前只有打字；可接既有 ASR，比照 `SpeakView` 的降級鏈，
   無 ASR 時退回打字）；②不記入学習記録（同 v3.29，沒加 `activity.ts` EXTRA_FEATURES，故不影響
   金印）——若要記，加一個 feature key、同步 `FEATURE_LABEL` 與 `TodayView` 的 `EXTRAS` 輪替並補測試；
...
3. ~~**跟讀＋即時追問**~~（v3.31 完成：`components/FollowUp.tsx`＋`lib/followUp.ts`，跟読分頁例句卡
   ...
   **可續做的小增量**：①口說回答（目前只有打字；可接既有 ASR 降級鏈，比照 `SpeakView` 的
   whisper▸瀏覽器 ASR▸自評，無 ASR 時退回打字）——與第 2 項的①同一件事，可一併做；
   ②不記入学習記録（同 v3.29／v3.30 未加 `activity.ts` EXTRA_FEATURES）；③追問目前不接續多輪
   （每次追問彼此獨立、不帶前一輪回答的上下文），若要做成小型多輪對話可沿用 `roleplay.ts`
   的 `roleplayHistory` 模式；④只在跟読分頁，会話（`DialogueView`）走完一段後也可比照追問。
4. **文型ドリル「自由造句」評分**：回想テスト已是「先想再看答案」的主動產出，可再進一步——讓你
   打字造一個用該句型的自己版本句子，Gemini 給中文講評（符不符合句型、有無明顯錯誤），僅供參考。
...
- **測試**：`npm test` 289/289、`npm run test:e2e` 58/58、`sidecar/test_score.py` 4/4、`test_article.py` 13/13、`npm run build` strict 綠燈。
