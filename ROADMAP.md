...
> 生成式的來回互動目前有三處——AI 助教的自由聊天與「考我」主動造句（`TutorView`，v3.30）、
> 以及自由対話（`RoleplayView`，v3.29 情境角色扮演）。這個方向要把「來回互動」的模式擴散到
> 更多練習場景。
...
- **測試**：`npm test` 262/262、`npm run test:e2e` 56/56、`sidecar/test_score.py` 4/4、`test_article.py` 13/13、`npm run build` strict 綠燈。
