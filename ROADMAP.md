...
最後更新：v3.46（文型ドリル「回想テスト」改為一輪制＋只練沒說對的——ROADMAP #6 點名「仍未涵蓋
`PatternView`」的續做，但真正的破口不只是缺進度條：回想テスト原本是**無止盡的循環**（`idx` 一直 +1
繞回去），沒有一輪、沒有結束、沒有結算，對「每天只練 10 分鐘」的人等於**不知道什麼時候算練完**；
更實際的損失是自評「🔁 再一次」的那幾句只是排到隊尾——詞池大時繞完一圈前不會重逢、詞池小時又一直
重複，**自評結果等於沒被用到**。這次收成一輪制：一輪固定題數（`ROUND_SIZE`＝8，不足取全部）、
隨機不重複、沿用 v3.28 的 `ProgressBar`、答完出結算（說對 n / m ＋列出還沒說順的句子）並可
「🔁 只練沒說對的（n 句）」再開一輪或「▶ 再來一輪」回到整個詞池。純邏輯抽 `lib/patternRound.ts`
（`buildRound`／`roundSummary`／`missedItems`／`roundNote`，rng 以參數注入沿用 `lib/quiz.ts` 慣例）。
**刻意不套用 `ScoreReveal` 的 ◎／○／△ 等第徽章**——這裡的百分比是使用者**自評**而非系統評分，
同一個徽章不該在不同語境代表不同意思（即 #6 ① 的提醒），結算畫面明寫「以上是你自己的判斷，
不是系統評分」。練習／自由造句兩模式行為不變、`logActivity('pattern')` 時機不變、仍是選配加練
不卡蓋章、不動 Dexie schema、不新增 CSS）。
前一版 v3.45（拗音ドリル：33 音看字選音——ROADMAP 「2.5 五十音圖續做」的①。使用者是
...
- **測試**：`npm test` 684/684、`npm run test:e2e` 86/86、`sidecar/test_score.py` 4/4、`test_article.py` 13/13、`npm run build` strict 綠燈。
...
- v3.28 只做了「多題作答流程」的進度條與對錯動畫（六處：`QuizView`／`ListenView` 四型／`KanaView`
  ...
  ~~`PatternView`（文型ドリル／回想テスト）~~（v3.46 完成：回想テスト改為**一輪制**——一輪固定題數、
  沿用 `ProgressBar` 顯示進度、答完出結算並可「只練沒說對的」再開一輪；純函式 `lib/patternRound.ts`。
  **刻意沒用 `ScoreReveal` 徽章**，理由見下方 ①——自評 ≠ 評分）。
  **仍未涵蓋**：`DialogueView`（會話引導逐句完成時——8 句的進度目前只有 `n / 8` 文字，可延伸同一套
  `ProgressBar`；走完一整段時也可考慮一次性的完成回饋）。
- **v3.42 之後新浮現的可續做**：①`ScoreReveal` 目前只用在兩處分數，`lib/quiz` 的 N5 測驗結算分數
  ...
  **v3.46 已就此做出一個判例**：回想テスト的「說對 n / m」是**使用者自評**，故只用純文字結算與
  `roundNote()` 一句話提示、**不套等第徽章**，並在畫面上明寫「不是系統評分」。N5 測驗的百分比是
  **系統判定**的對錯，性質與自評不同，若要套徽章應另立一組門檻與標籤，別直接沿用 `WRITE_BANDS`／
  `SPEAK_BANDS`（那兩組是相似度分數的門檻）；
...
- **v3.46 之後新浮現的可續做**：ⓐ回想テスト的一輪結果**不持久化**（換頁就沒了）——若要在成長頁
  呈現「哪些句型最常說不順」需要 Dexie（比照 `quizResults`，version 9＋同步 `e2e/db.spec.ts` 版本斷言）；
  先觀察「當下就能只練沒說對的」夠不夠用，別為了功能而功能。ⓑ`buildRound` 目前是均勻隨機取樣，
  沒有「上一輪沒說對的下一輪優先出」的跨輪權重——同一個模組加得上去，但同樣要先有持久化才有意義。
  ⓒ文型ドリル「練習」模式仍是無止盡的「換一個單字 →」（那是刻意的：純聽讀熟悉、隨時可走，
  不需要收尾），若日後覺得也該有節奏感再議。
