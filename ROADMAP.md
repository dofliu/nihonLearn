...
最後更新：v3.27（段落聽解細節題——「段落對話」除大意/場景理解題外，補上 6 篇短文的時間/數量/人物
細節題，答案由短文 zh 台詞逐字支持並有測試核對）。另有 PR #32（v3.26 漢字筆順「行筆方向」比對）
尚未合併，待合併後 CLAUDE.md 版本序可能需人工理順（兩支獨立分支各自延續 v3.25，不互相依賴）。
...
| 段落聽解細節題（`data/passages.ts` `detailQuiz`，時間/數量/人物，答案逐字對照 zh 台詞並有測試核對） | v3.27 |
