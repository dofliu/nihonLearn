 ...
 *    塞入既有資料，再載入 app 觸發升級到現行 version(5)（版本 50），驗證資料不遺失、
 *    新表 userSentences / ttsCache / userPassages / genQueue / quizResults 建立。
...
test.describe('Dexie 升級與 v1 匯入', () => {
  test('v1 → 現行 schema 升級不遺失既有資料', async ({ page }) => {
    ...
    // schema 已升級：IDB 版本 50（Dexie version(5)×10）、新表建立
